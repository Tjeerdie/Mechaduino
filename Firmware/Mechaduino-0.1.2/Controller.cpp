...
void TC5_Handler() {                // gets called with FPID frequency
  ...
  static int SamplesCounter = 0;               //this is used by step response
  static unsigned int SkipCounter = 0;               //this is used by step response
  ...
  if (TC5->COUNT16.INTFLAG.bit.OVF == 1) {    // A counter overflow caused the interrupt
   ...
   // u_3 = u_2;
    ...
    SkipCounter += 1;
    if (SkipCounter >= 1 and SamplesCounter<=LoggerSize){    // log every 5th loop (every time is too much data to log and stop logging when running out of logging places
      if (log_velocity or log_position or log_torque){ //for step resonse... still under development
         log_yw[SamplesCounter] = yw;      //for step response, under development...
         log_v[SamplesCounter] = v;      //for step response, under development...
         log_u[SamplesCounter] = u;      //for step response, under development...
         log_r[SamplesCounter] = r;      //for step response, under development...
         SamplesCounter += 1;
      }
      SkipCounter = 0;
    }
